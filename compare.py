...
class ImageMatching(object):
    ...
    def find_best_result(self):
        ...
        # Get the feature point set and match the feature point pair: Return value(good, pypts, kp_sch, kp_src)
        self.kp_sch, self.kp_src, self.good = self._get_key_points()

        # Extract the recognition area based on the matching point pair:
        if len(self.good) in [0, 1]:
            # The matching point pair is 0, and the recognition area cannot be extracted;
            # if it is 1, the target area cannot be obtained, and None is directly returned as the matching result:
            return None
        elif len(self.good) in [2, 3]:
            # The matching point pair is 2 or 3, and the target area is obtained based on the point pair,
            # and the credibility is calculated based on this:
            if len(self.good) == 2:
            ...
            # n some special cases, return None directly as the matching result:
            if origin_result is None:
                return origin_result
            else:
                middle_point, pypts, w_h_range = origin_result
        else:
            # Matching point pairs> = 4, use single matrix mapping to find the target area,
            # and calculate the credibility based on this：
    ...
    def _many_good_pts(self, kp_sch, kp_src, good):
        ...
        # Picking out the target rectangular area may have an out-of-bounds situation, and when it is out-of-bounds,
        # it is directly set as the boundary:
        # 0 beyond the left boundary, w_s-1 beyond the right boundary,
        # 0_ beyond the lower boundary, h_s-1 beyond the upper boundary1
        # When x_min is less than 0, take 0. When x_max is less than 0, 0 is taken.
        ...
        # When y_min is less than 0, 0 is taken. When y_max is less than 0, 0 is taken.
        y_min, y_max = int(max(y_min, 0)), int(max(y_max, 0))
        # When y_min is greater than h_s, the value is h_s-1. When y_max is greater than h_s-1, h_s-1 is taken.
        y_min, y_max = int(min(y_min, h_s - 1)), int(min(y_max, h_s - 1))
        # Corner points of the target area, in the order of upper left, lower left, lower right, upper right:
    ...
    def _find_homography(self, sch_pts, src_pts):
        ...
        try:
            ...
            import traceback
            traceback.print_exc()
            raise _robot_error("OpenCV error in _find_homography()...")
        else:
            if mask is None:
                raise _robot_error("In _find_homography(), find no transfomation matrix...")
            else:
                return M, mask
    ...
    def _target_error_check(self, w_h_range):
        ...
        # If the number of pixels of the width and height of the rectangular recognition area in src_img is less than 5,
        # it is determined that the recognition is invalid. It is considered that the extraction area cannot be smaller
        # than 5 pixels. (Screenshots are generally not smaller than 5 pixels)
        if tar_width < 5 or tar_height < 5:
            raise _robot_error("In src_image, Taget area: width or height < 5 pixel.")
        ...
        if tar_width < 0.2 * w or tar_width > 5 * w or tar_height < 0.2 * h or tar_height > 5 * h:
            raise _robot_error("Target area is 5 times bigger or 0.2 times smaller than sch_img.")


def _robot_error(error):
    BuiltIn().fail(error)
